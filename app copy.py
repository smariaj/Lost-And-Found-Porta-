...
def dashboard():
    ...
    for item in items:
        text_matches = find_text_matches(item)
        items_with_matches.append({'item': item, 'matches': text_matches})
...
def admin_dashboard():
    ...
    # for f_item in found_items:
    #     text_matches = find_text_matches(f_item)
    #     found_items_with_matches.append({'item': f_item, 'matches': text_matches})

    found_items_with_matches = [{'item': f, 'matches': []} for f in found_items]
...
def add_item():
    ...
    if request.method == 'POST':
        ...
        matches = find_text_matches(new_item)
...
#             recipients=["karpevaishnavi08@gmail.com"],
#             body="Mail configuration is working! Let's gooo!"
#         )
#         mail.send(msg)
